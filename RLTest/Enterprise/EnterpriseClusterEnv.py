...
class EnterpriseClusterEnv():
    ...
    def __init__(self, shardsCount=1, redisBinaryPath=None, modulePath=None, moduleArgs=None, logFileFormat=None,
                 dbFileNameFormat=None, dbDirPath=None, useSlaves=False, dmcBinaryPath=None):
        ...
        for i in range(0, totalRedises, (2 if useSlaves else 1)):
            shard = OssEnv(redisBinaryPath=redisBinaryPath, port=startPort, modulePath=self.moduleSoFilePath, moduleArgs=self.moduleArgs,
                           ...
                           serverId=(i + 1), password=SHARD_PASSWORD)
            self.shards.append(shard)
            startPort += 2

        self.ccs = CcsMock(redisBinaryPath=redisBinaryPath, directory=dbDirPath, useSlaves=useSlaves,
                           password=SHARD_PASSWORD, proxyPort=self.DMC_PORT)
        self.dmc = Dmc(directory=dbDirPath, dmcBinaryPath=dmcBinaryPath)
